...
namespace JetBrains.Debugger.Worker.Plugins.Unity.Values.Render.ChildrenRenderers
{
    ...
    public class MonoSerializedObjectChildrenRenderer : SerializedObjectChildrenRenderer<Value>
    {
        public MonoSerializedObjectChildrenRenderer(IUnityOptions unityOptions, ILogger logger) : base(unityOptions, logger)
        {
        }
    }

    [DebuggerSessionComponent(typeof(CorDebuggerType))]
    public class CorSerializedObjectChildrenRenderer : SerializedObjectChildrenRenderer<ICorValue>
    {
        public CorSerializedObjectChildrenRenderer(IUnityOptions unityOptions, ILogger logger) : base(unityOptions, logger)
        {
        }
    }
        ...
        where TValue : class
    {
        ...
        protected SerializedObjectChildrenRenderer(IUnityOptions unityOptions, ILogger logger)
        {
        ...
        private class ChildrenGroup : ValueGroupBase
        {
            ...
            private IValueEntity[] GetChildrenImpl(IValueFetchOptions options)
            {
                ...
                // TODO: this is kinda annoying UI-wise, since all SerializedObject will always have a single "Base"
                //  entry that the user will have to expand further to get more properties. should we skip it?
            }
