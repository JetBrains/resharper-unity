    ...
    public static IEnumerable<SubplatformFileForPackaging> CompileEditorPlugin(AllAssembliesOnEverything allAss , ProductHomeDirArtifact homeDirArtifact, ILogger logger)
    {
        if (allAss.FindSubplatformByClass<CompileDebuggerToolsBuildStep>() is SubplatformOnSources subplatform)
        {
            ...
            return new SubplatformFileForPackaging[]
            {
                ...
                new(subplatform.Name, ImmutableFileItem.CreateFromDisk(textureUtilsDotNetCoreOutputFolder / "JetBrains.ReSharper.Plugins.Unity.Rider.Debugger.Presentation.Texture.DotNetCore.dll")),
                new(subplatform.Name, ImmutableFileItem.CreateFromDisk(textureUtilsDotNetCoreOutputFolder / "JetBrains.ReSharper.Plugins.Unity.Rider.Debugger.Presentation.Texture.DotNetCore.pdb")),
