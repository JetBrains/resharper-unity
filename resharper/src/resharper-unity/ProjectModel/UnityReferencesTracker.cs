...
#if RIDER
            var changesWithAssemblies = ReferencedAssembliesService.TryGetAssemblyReferenceChanges(projectModelChange, ProjectExtensions.UnityReferenceNames);
            var changes = changesWithAssemblies.Select(kv => kv.Key);
#endif
#if WAVE08
