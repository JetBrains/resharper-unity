...
public class UnityObjectLifetimeCheckViaNullEqualityHintAdornmentProvider : IHighlighterAdornmentProvider
{
    public bool IsValid(IHighlighter highlighter) =>
        highlighter.GetHighlighting() is UnityObjectNullComparisonHintHighlighting hint && hint.IsValid();
    ...
    public IAdornmentDataModel? CreateDataModel(IHighlighter highlighter)
    {
        ...
        {
            var data = new AdornmentData(hint.Text, hint.Icon, AdornmentFlags.IsNavigable,
                new AdornmentPlacement(UnityObjectNullComparisonHintHighlighting.DefaultOrder,
                    AdornmentPosition.INLINE), hint.Mode);
    ...
    class DataModel(AdornmentData data, IEqualityExpression expression) : IAdornmentDataModel
    {
        ...
        public IEnumerable<BulbMenuItem> ContextMenuItems => EmptyList<BulbMenuItem>.InstanceList;
