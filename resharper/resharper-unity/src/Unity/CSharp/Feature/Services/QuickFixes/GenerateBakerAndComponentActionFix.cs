...
{
    public class GenerateBakerAndComponentActionFix : WorkflowDrivenImplementMembersFix<GenerateBakerAndComponentActionWorkflow>
    {
        [CanBeNull] private readonly ITreeNode myAnchor;

        public GenerateBakerAndComponentActionFix(IClassLikeDeclaration typeDeclaration, ITreeNode anchor = null)
            : base(typeDeclaration)
        {
            myAnchor = anchor;
        }

        public override string Text => Strings.UnityDots_GenerateBakerAndComponent_Name;

        protected override GenerateBakerAndComponentActionWorkflow TryCreateWorkflow()
        {
            return new GenerateBakerAndComponentActionWorkflow();
        }
