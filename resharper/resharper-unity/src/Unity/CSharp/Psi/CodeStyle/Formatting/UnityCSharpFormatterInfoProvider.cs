...
namespace JetBrains.ReSharper.Plugins.Unity.CSharp.Psi.CodeStyle.Formatter
{
    ...
    public class UnityCSharpFormatterInfoProvider : CSharpFormatterInfoProviderPart
    {
        ...
        private void HeaderAttributeRules()
        {
            var singleHeaderAttributeSection = Node().In(ElementType.ATTRIBUTE_SECTION).Satisfies((node, _) =>
                myPossibleHeaderNames.Contains(GetSingleAttributeSectionName(node)) && node.GetProject().HasUnityFlavour()).Obj;

            var singleHeaderAttributeSectionList = Node().In(ElementType.ATTRIBUTE_SECTION_LIST).Satisfies((node, _) =>
                myPossibleHeaderNames.Contains(GetSingleSectionAttributeListName(node)) && node.GetProject().HasUnityFlavour()).Obj;
            ...
            var attributeSectionFollowingHeader = Node().In(ElementType.ATTRIBUTE_SECTION).Satisfies((node, _) =>
            {
                ...
                return previousSection != null && myPossibleHeaderNames.Contains(GetSingleAttributeSectionName(previousSection)) && previousSection.GetProject().HasUnityFlavour();
            }).Obj;
