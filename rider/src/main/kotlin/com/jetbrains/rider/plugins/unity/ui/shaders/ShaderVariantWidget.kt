        ...
        shaderVariant.info.advise(lifetime) { info ->
            val newText = when {
                info.suppressedCount > 0 -> {
                    val strikethrough = info.suppressedCount.toString().map { "$it\u0336" }.joinToString("")
                    UnityBundle.message("shaderVariant.widget.text.with.suppressed", info.enabledCount,
                                        strikethrough, info.availableCount)
                }
