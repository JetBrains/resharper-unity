...
class ShaderVariantPresence(project: Project?, markupModel: MarkupModelEx) : Disposable {
    companion object {
        ...
        private const val ACTIVE_KEYWORD_EXTERNAL_NAME = "ReSharper.ReSharper.ShaderLab_ENABLED_SHADER_KEYWORD"
    ...
    val enabledKeywords = CountingSet<String>()
        ...
        markupModel.addMarkupModelListener(this, object : MarkupModelListener {
            override fun afterAdded(highlighter: RangeHighlighterEx) {
                ...
                when (info.type.attributesKey.externalName) {
                    ...
                    ACTIVE_KEYWORD_EXTERNAL_NAME -> addEnabledKeywordHighlighter(highlighter)
                }
            }
            ...
            override fun afterRemoved(highlighter: RangeHighlighterEx) {
                ...
                when (info.type.attributesKey.externalName) {
                    ...
                    ACTIVE_KEYWORD_EXTERNAL_NAME -> removeEnabledKeywordHighlighter(highlighter)
                }
            }
        })
    }
    ...
    private fun addEnabledKeywordHighlighter(highlighter: RangeHighlighterEx) {
        if (enabledKeywords.add(highlighter.text, 1) == 1)
            fireChange()
    }

    private fun removeEnabledKeywordHighlighter(highlighter: RangeHighlighterEx) {
        if (enabledKeywords.add(highlighter.text, -1) == 0)
            fireChange()
    }
