...
internal class TextureDebuggerCollector : CounterUsagesCollector() {
    companion object {
        ...
        fun prepareTextureToShowStarted(project: Project?,
                                        parentActivity: StructuredIdeActivity?,
                                        width: Int,
                                        height: Int): StructuredIdeActivity? {
            if (parentActivity == null)
                return null
            return PREPARE_TEXTURE_PIXELS_TO_SHOW_OPERATION.startedWithParent(project, parentActivity) {
                listOf(TEXTURE_WIDTH.with(width), TEXTURE_HEIGHT.with(height))
            }
        }
